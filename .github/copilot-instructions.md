...
This is a **customer portal bootstrap** — a pnpm monorepo with 4 packages that build a multi-tenant SaaS portal with Entra ID authentication, Stripe billing, and MCP server for AI agent management.
...
packages/
  ...
  mcp-server/→ MCP server with Entra Workforce OAuth (optional)
```
...
- `@initialise` — First-time project setup from bootstrap template
...
- **dev** → local development + Azure dev resources
- **staging** (optional) → pre-production verification
- **production** → live customer-facing environment

Environment config in `infra/parameters.{env}.json`. Secrets in GitHub Actions environment secrets.
